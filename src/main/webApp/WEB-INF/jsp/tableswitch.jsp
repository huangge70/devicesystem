...
<div id="wrapper">
    <nav class="navbar navbar-default top-navbar" role="navigation">
        <div class="navbar-header">
            ...
            <a class="navbar-brand" href="index.html"><i class="fa fa-gear"></i> <strong>设备管理系统</strong></a>
        </div>
            ...
            <ul class="nav" id="main-menu">
                ...
                <li>
                    ...
                    <ul class="nav nav-second-level">
                        ...
                        <li>
                            <a href="/devicesystem/device/getSwitch" class="active-menu">交换机</a>
                        </li>
                        <li>
                        ...
                        <div class="panel-body">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                    ...
                                    <tbody>
                                    <c:forEach items="${swList}" var="swboard">
                                        <tr class="odd gradeX">
                                            <td class="center">${swboard.switchboardId}</td>
                                            <td class="center">${swboard.switchboardBrand}</td>
                                            <td class="center">${swboard.switchboardPortnumber}</td>
                                            <td>${swboard.switchboardInterfacetype}</td>
                                            <td><fmt:formatDate value="${swboard.switchboardStoragetime}" pattern="yyyy-MM-dd"/></td>
                                            <c:if test="${swboard.switchboardStatus=='1'}">
                                                <td>在库</td>
                                            </c:if>
                                            <c:if test="${swboard.switchboardStatus=='0'}">
                                                <td>借出</td>
                                            </c:if>
                                            <c:if test="${swboard.switchboardStatus=='-1'}">
                                                <td>损坏</td>
                                            </c:if>
                                            <td>${swboard.switchboardLendername}</td>
                                            <td>${swboard.switchboardLendertel}</td>
                                            <td><fmt:formatDate value="${swboard.switchboardBorrowtime}" pattern="yyyy-MM-dd"/></td>
                                            <c:if test="${swboard.switchboardStatus=='1'}">
                                                <td><a href="/devicesystem/deviceorder/insertOrder?deviceId=${swboard.switchboardId}&deviceNumber=${swboard.switchboardBrand}&deviceType=switch">借设备</a></td>
                                            </c:if>
                                            <c:if test="${swboard.switchboardStatus=='0'}">
