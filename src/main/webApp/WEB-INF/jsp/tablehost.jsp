            ...
            <ul class="nav" id="main-menu">
                ...
                <li>
                    ...
                    <ul class="nav nav-second-level">
                        ...
                        <li>
                            <a href="/devicesystem/device/getHost" class="active-menu">主机</a>
                        </li>
                        <li>
                                ...
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                    ...
                                    <c:forEach items="${hoList}" var="host">
                                        <tr class="odd gradeX">
                                            ...
                                            <c:if test="${host.hostStatus=='1'}">
                                                <td>在库</td>
                                            </c:if>
                                            <c:if test="${host.hostStatus=='0'}">
                                                <td>借出</td>
                                            </c:if>
                                            <c:if test="${host.hostStatus=='-1'}">
                                                <td>损坏</td>
                                            </c:if>
                                            <td>${host.hostLendername}</td>
                                            <td>${host.hostLendertel}</td>
                                            <td><fmt:formatDate value="${host.hostBorrowtime}" pattern="yyyy-MM-dd"/></td>
                                            <c:if test="${host.hostStatus=='1'}">
                                                <td><a href="/devicesystem/deviceorder/insertOrder?deviceId=${host.hostId}&deviceNumber=${host.hostBrand}&deviceType=host">借设备</a></td>
                                            </c:if>
                                            <c:if test="${host.hostStatus=='0'}">
