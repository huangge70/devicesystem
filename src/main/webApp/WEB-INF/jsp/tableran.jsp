            ...
            <ul class="nav" id="main-menu">
                ...
                <li>
                    ...
                    <ul class="nav nav-second-level">
                        ...
                        <li>
                            <a href="/devicesystem/device/getRan" class="active-menu">测距仪</a>
                        </li>
                    </ul>
                </li>
                <li>
                        ...
                        <div class="panel-body">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                    ...
                                    <tbody>
                                    <c:forEach items="${ranList}" var="ran">
                                        <tr class="odd gradeX">
                                            <td class="center">${ran.ranId}</td>
                                            <td class="center">${ran.ranBrand}</td>
                                            <td class="center">${ran.ranAccuracy}</td>
                                            <td class="center">${ran.ranDistance}</td>
                                            <td>${ran.ranMethod}</td>
                                            <td><fmt:formatDate value="${ran.ranStoragetime}" pattern="yyyy-MM-dd"/></td>
                                            <td>${ran.ranPrice}</td>
                                            <c:if test="${ran.ranStatus=='1'}">
                                                <td>在库</td>
                                            </c:if>
                                            <c:if test="${ran.ranStatus=='0'}">
                                                <td>借出</td>
                                            </c:if>
                                            <c:if test="${ran.ranStatus=='-1'}">
                                                <td>损坏</td>
                                            </c:if>
                                            <td>${ran.ranLendername}</td>
                                            <td>${ran.ranLendertel}</td>
                                            <td><fmt:formatDate value="${ran.ranBorrowtime}" pattern="yyyy-MM-dd"/></td>
                                            <c:if test="${ran.ranStatus=='1'}">
                                                <td><a href="/devicesystem/deviceorder/insertOrder?deviceId=${ran.ranId}&deviceNumber=${ran.ranBrand}&deviceType=ran">借设备</a></td>
                                            </c:if>
                                            <c:if test="${ran.ranStatus=='0'}">
